...
| OIDC assumption fails | Check provider/audience, exact subject (repo/main or environment), role ARN, environment name, workflow permissions, and organization SCPs. |
...
| Apply fails after SSM update | Workflow attempts safe parameter restoration and reports the SHA. Inspect state/services, then generate and approve a new apply—no manual/unreviewed rollback. |
