...
5. Terragrunt creates a readable and exact binary plan plus SHA-256 checksum, then restores the previous SSM state while approval is pending.
6. A protected environment pauses execution for review.
7. A new OIDC session verifies account and checksum, writes the approved image references, then applies only the saved plan.
8. ECS rolls out both task definitions and the workflow smoke-tests the ALB paths.
