...
- OIDC avoids long-lived keys. Trust policies must pin the immutable owner/repository identity, branch, and audience; IAM/pass-role scopes must match project resources.
...
- State and binary/readable plans can disclose infrastructure. Use encrypted/versioned S3, short artifact retention, restricted repository access, and checksum verification.
- Destroy requires an exact phrase. GitHub Environment approvals are intentionally disabled for now, so repository write access is the deployment boundary. ALB deletion protection can intentionally block destruction.
